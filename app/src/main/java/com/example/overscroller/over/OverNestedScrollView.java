...
public class OverNestedScrollView extends FrameLayout implements NestedScrollingParent,
    ...
    public OverNestedScrollView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //over scroll 300dp
        mOverScrollDistance = (int) (context.getResources().getDisplayMetrics().density * 300);
        //over fling 150dp
        mOverFlingDistance = (int) (context.getResources().getDisplayMetrics().density * 200);
    }
        ...
        switch (actionMasked) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsBeingDragged) {
                    ...
                    if (oldY <= 0 && deltaY < 0) {
                        float scrollRate = getScrollRate(oldY, mOverScrollDistance);
                        deltaY *= scrollRate;
                    }
                    if (oldY >= range && deltaY > 0) {
                        deltaY *= getScrollRate(oldY - range, mOverScrollDistance);
                    }
    ...
    protected void onOverScrolled(int scrollX, int scrollY,
        ...
        if (!mScroller.isFinished()) {
            ...
            if (clampedY) {
                mScroller.springBack(this.getScrollX(), this.getScrollY(), 0, 0, 0, getScrollRange());
            }
        } else {
            super.scrollTo(scrollX, scrollY);
        }
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (dy != 0) {
                ...
                overScrollByCompat(0, dy, getScrollX(), oldScrollY, 0, range, 0, mOverScrollDistance, false);
